...
contract Pausable is Ownable {
    ...
    function setSetPaused(bool pause) public onlyOwner {
        _paused = pause;
    }
...
contract ERC721 is Pausable, ERC165 {
    ...
    modifier requireAddressIsNotEmpty(address adres) {
        require(adres != address(0), 'address is empty');
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, 'token already receivers one');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner || isApprovedForAll(owner,msg.sender), 'owner or approved operate is not owner of this token');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal
        requireAddressIsNotEmpty(to)
     {
        ...
        require(from == ownerOf(tokenId), 'token is not owned by from');
...
contract RealEstateNdERC721Token is ERC721Metadata("Real Estate ND","RND","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
